...
public class UIUtils {
	...
	public static String getTransactionSaerchPage(TxnSearchResponse txnSearchResponse) {
		...
		for (TxnDetailType txn : txnSearchResponse.getTxnDetails()) {
			...
			sb.append("<th style=\"padding-left: 5px;width:1%;\"  onclick=\"enableView()\" scope=\"row\"><input type=\"radio\" name=\"txnlist\" value=\"" + txn.getTxnReferenceId()
					+ "\"></th>\n");
			sb.append("<td style=\"width:10%;\" id='"+txn.getTxnReferenceId()+"_txnId' >" + txnId + "</td>\n");
			sb.append("<td style=\"width:11%;\" id='"+txn.getTxnReferenceId()+"_billerId' >" + txn.getBillerId() + "</td>\n");
			// String blrName=null;
...
//			sb.append("<td style=\"width:13%;\">" + "Undefined" + "</td>\n");
	...
	public static String getComplaintStatus(ComplaintRaisedResponse resp) {
		...
		if (resp != null) {
			...
			if (resp.getResponseCode() != null && resp.getResponseCode().equals("001")) {
				sb.append("<font color=\"red\" style=\"font-size: 18px;font-family: roboto;\">");
				sb.append("<center>");
			...
			if (resp.getErrors() != null && resp.getErrors().size() > 0) {
				for (in.co.rssoftware.bbps.schema.ErrorMessage message : resp.getErrors()) {
					sb.append("<font color=\"red\" style=\"font-size: 18px;font-family: roboto;\">");
					sb.append("<center>");
					sb.append(message.getErrorCd() + ": " + message.getErrorDtl());
					sb.append("</font></center><br>");
				}				
			}
		}
