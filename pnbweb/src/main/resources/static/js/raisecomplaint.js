...
function viewTransactionDetails()
{
	...
	$('#mob_row').hide();
	 var srchBy = $('#srch_by').val();
       var txnID=$('input[name=txnlist]:checked').val();
       var mobNo = $('#'+$('input[name=txnlist]:checked').val()+'_mob').val();
...
function resendOTPReq(comp_type)
{
       ...
       if(comp_type=='tran')
              {
              comp_type='COMP_TRAN';
              mobile =$('#'+$('input[name=txnlist]:checked').val()+'_mob').val();
...
//                   $("#loader").show();
                     ...
                     $.ajax({
                        ...
                        if(results.searchBy=='TRAN_REF_ID')
                           {
                           
                           $("#srch_by").val('TRAN_REF_ID');
                           $("#otpBox").hide();
                           $("#mobbox").hide();
                           document.getElementById('tran_comp_sub').removeAttribute('disabled');
                           }
...
if(typeof val !="undefined"){
       ...
       $("#complainttxnid").val($('input[name=txnlist]:checked').val());
       $("#complainttxnid_view").val($('#'+$('input[name=txnlist]:checked').val()+'_txnId').html());
       var srch_by=document.getElementById("srch_by").value;
       if(srch_by!='TRAN_REF_ID')
    	   {
    	   $("#complaintmobile").val($('#'+$('input[name=txnlist]:checked').val()+'_mob').val());
    	   $("#comp_mob").show();
    	   }
...
//                element6.name="otpInput";
               ...
               if(srch_by=='TRAN_REF_ID')
                     ...
                     if(isEmpty(mob))
                           ...
                           {
                     isvalidated=validateOTP(otpInput,mob);
                           }
                     }
              if(isvalidated)
                     {
...
//                       form.appendChild(element6);
              ...
              else if(isNullOrEmpty(otpInput)) {
            	  $("#tranOtp_errormsgcont").html("");
...
function statusCheckRequest(){
       //     alert("Checking status......");
              ...
              if(complaintType==""){
            	  ...
            	  alert("Please provide complaintId ");
            	  }
            	  else{
