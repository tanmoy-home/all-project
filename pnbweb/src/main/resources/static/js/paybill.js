...
function showCustomerInfoArea(biller) {
	if (biller.value != "") {
		...
		if (biller.options[biller.selectedIndex].getAttribute('data-accept-adhoc') == 1)
			$("#quickPayGroup").show();			
		else
			$("#quickPayGroup").hide();
	} else {
...
function quickPay() {
	...
	var quickPaymentmethod = document.getElementById('quickPaymentmethod');
	if(quickPaymentmethod.options[quickPaymentmethod.selectedIndex].value==='')
		{
		alert('Please select Payment method .');
		return false;
		}
...
function appendPayForm() {
	...
	var billerAmountOptions = document.getElementById('billerAmountOptions');
	if(billerAmountOptions.options[billerAmountOptions.selectedIndex].value==='')
		{
