...
public class ComplaintController {
	...
	public String raiseComplaintRequestView(@ModelAttribute("userDetails") UserDetails userDetails, Model model,
		...
		model.addAttribute("raiseComp_selected", "active ");
	...
	public String checkComplaintStatusView(@ModelAttribute("userDetails") UserDetails userDetails, Model model,
		...
		model.addAttribute("userDetails", userDetails);
		model.addAttribute("compsttus_selected", "active ");
	...
	public String checkComplaintStatus(HttpServletRequest request, HttpServletResponse response,
			@ModelAttribute("userDetails") UserDetails userDetails, @PathVariable String tenantId, @PathVariable String complaintType,
			@PathVariable String complaintId) throws ValidationException {
		...
		try {
			ResponseEntity responseEntity = internalRestTemplate.exchange(uri + WebAPIURL.COMPLAINT_CHECK_STATUS_URL, HttpMethod.POST,
	...
	public String getServicereasonList(@ModelAttribute("userDetails") UserDetails userDetails, HttpServletRequest request,
			HttpServletResponse response, @PathVariable String tenantId, @PathVariable String particiationType) throws ValidationException {
		...
		try {
			ResponseEntity responseEntity = internalRestTemplate.exchange(uri + WebAPIURL.COMPLAINT_ALL_SERVICE_REASON_URL, HttpMethod.POST,
	...
	public String getAgentList(@ModelAttribute("userDetails") UserDetails userDetails, HttpServletRequest request,
		...
		try {
			ResponseEntity responseEntity = internalRestTemplate.exchange(uri + WebAPIURL.FETCH_ALL_AGENT_URL, HttpMethod.GET,
					Utility.getHttpEntityForGet(), AgentList.class, tenantId);
	...
	public String getBillerList(@ModelAttribute("userDetails") UserDetails userDetails, HttpServletRequest request,
		...
		try {
			ResponseEntity responseEntity = internalRestTemplate.exchange(uri + WebAPIURL.FETCH_ALL_BILLER_URL, HttpMethod.GET,
					Utility.getHttpEntityForGet(), BillerList.class, tenantId);
	...
	public String raiseServiceComplaint(@ModelAttribute("userDetails") UserDetails userDetails, HttpServletRequest request,
			HttpServletResponse response, @PathVariable String tenantId, @RequestParam String servReason,
			@RequestParam String participationType, @RequestParam String participationId, @RequestParam String description,
			@RequestParam String typeName, Model model) throws ValidationException {
		...
		try {
			...
			if (resp.getErrors() != null && resp.getErrors().size() == 0) {
				...
				complaintReceiptDTO.setComplaintDate(CommonUtils.formaterdd_MM_YYYY_HH_MM_A.print(new DateTime(Calendar.getInstance())));
				...
				// getNotificationTemplate(NotificationEventType.SVC_COMP_RAISE.name(),
				// NotificationType.SMS.name());
				//
				// String notificationMsg = notificationTemplate
				// .replace(CommonConstants.TEMPLATE_COMP_ID,
				// resp.getComplaintId())
				// .replace(CommonConstants.TEMPLATE_DATE,
				// CommonUtils.formaterdd_MM_YYYY_HH_MM_A.print(new DateTime()))
				// .replace(CommonConstants.TEMPLATE_COMP_SVC_TYPE,
				// complaintReceiptDTO.getTransactionLabel());
				if (resp.getOpenComplaint() != null) {
					if (resp.getOpenComplaint().equals("Y")) {
						...
						complaintReceiptDTO.setResponseMsg(
								"Complaint already raised for this Transaction with Complaint ID: " + resp.getComplaintId());
				...
				for (ErrorMessage errorMessage : resp.getErrors()) {
					msg += errorMessage.getErrorCd() + " : " + errorMessage.getErrorDtl();
				}
				complaintReceiptDTO.setIsDup("Failed");
				complaintReceiptDTO.setResponseMsg(msg);
			}
		} catch (Exception e) {
			complaintReceiptDTO.setIsDup("Failed");
	...
	public TxnSearchDTO getTransactionList(@ModelAttribute("userDetails") UserDetails userDetails, HttpServletRequest request,
			HttpServletResponse response, @PathVariable String tenantId, @PathVariable String search_by,
			@RequestParam(value = "mobno", required = false) String mobNo, @RequestParam(value = "fromdt", required = false) String fromdt,
			@RequestParam(value = "todt", required = false) String todt, @RequestParam(value = "txnId", required = false) String txnId)
			throws ValidationException {

		log.info("In the method getTransactionList of ComplaintController.");

		String errorMsgPart = "mobile number in given date range.";
		TransactionContext.putTenantId(tenantId);
		...
		ResponseEntity responseEntity = null;
		Map<String, String> billerMap = new HashMap<String, String>();
		BillerList billerList = null;
		try {
			responseEntity = internalRestTemplate.exchange(uri + WebAPIURL.FETCH_ALL_BILLER_URL, HttpMethod.GET,
					Utility.getHttpEntityForGet(), BillerList.class, tenantId);
			billerList = (BillerList) responseEntity.getBody();
			if (billerList != null && billerList.getBillers() != null) {
				for (BillerDetail billerView : billerList.getBillers()) {
					billerMap.put(billerView.getId(), billerView.getName());
				}
			}
		...
		try {
			responseEntity = internalRestTemplate.exchange(uri + WebAPIURL.FETCH_ALL_TRANSACTIONS_LIST_URL, HttpMethod.POST,
					Utility.getHttpEntityForPost(req), TxnSearchResponse.class, tenantId);
		...
		if (resp != null && resp.getErrors().size() != 0) {
			...
			txnSearchDTO.setTxnlistAsString(UIUtils.getTransactionSaerchPage(resp, billerMap,mobNo));
			txnSearchDTO.setSearchBy(search_by);
	...
	public String getDispositionList(@ModelAttribute("userDetails") UserDetails userDetails, HttpServletRequest request,
		...
		try {
			ResponseEntity responseEntity = internalRestTemplate.exchange(uri + WebAPIURL.FETCH_ALL_DISPOSITION_URL, HttpMethod.GET,
					Utility.getHttpEntityForGet(), DispositionList.class, tenantId);
	...
	public String raiseTxnComplaint(@ModelAttribute("userDetails") UserDetails userDetails, HttpServletRequest request,
			HttpServletResponse response, @PathVariable String tenantId, @RequestParam String txnRefId, @RequestParam String disposition,
		...
		ResponseEntity responseEntity = null;
		try {
			responseEntity = internalRestTemplate.exchange(uri + WebAPIURL.RAISE_TRANSACTION_COMPLAINT_URL, HttpMethod.POST,
					Utility.getHttpEntityForPost(complainRequest), ComplaintRaisedResponse.class, tenantId);
			resp = (ComplaintRaisedResponse) responseEntity.getBody();
			if (resp.getErrors() != null && resp.getErrors().size() == 0) {
				complaintReceiptDTO.setComplaintId(resp.getComplaintId());
				...
				// + "> against Transaction ref-Id <" + txnRefId + "> on " +
				// dttm +
				// ".";
				String notificationTemplate = getNotificationTemplate(NotificationEventType.TRAN_COMP_RAISE.name(),
						NotificationType.SMS.name());

				String notificationMsg = notificationTemplate.replace(CommonConstants.TEMPLATE_COMP_ID, resp.getComplaintId())
	...
	private String getNotificationTemplate(String eventType, String notificationType) {
		...
		ResponseEntity<?> responseEntity = internalRestTemplate.exchange(apiUrl, HttpMethod.GET, Utility.getHttpEntityForGet(),
				String.class, tenantId, notificationType, eventType);
	...
	private boolean isValidated(UserDetails userDetails) {
		...
		try {
			ResponseEntity<Ack> responseEntity = internalRestTemplate.postForEntity(testURL, Utility.getHttpEntityForGet(), Ack.class);
