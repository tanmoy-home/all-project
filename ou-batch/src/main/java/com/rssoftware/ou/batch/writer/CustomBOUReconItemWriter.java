...
public class CustomBOUReconItemWriter implements ItemWriter<ReconBOU>{
	private static Logger log = LoggerFactory.getLogger(CustomBOUReconItemWriter.class);
	...
	@Autowired
	BillerOUtransactionDataDaoImpl transactionDataDao;
	@Autowired
	...
	private ReconDetails formReconDetailsFromTD(ReconBOU item) {
	...
	private ReconDetails formReconDetailsFromRD(ReconBOU item) {
		ReconDetails reconDetails = new ReconDetails();
		ReconDetailsPK reconDetailsPK = new ReconDetailsPK();
	...
	private BillerOUtransactionData formTransactionData(ReconBOU item) {
		TransactionDataPK id = new TransactionDataPK();
		id.setRefId(item.getRefId());
		id.setTxnType(item.getTxnType());
		BillerOUtransactionData transactionData = transactionDataDao.get(id);
		transactionData.setReconTs(new Timestamp(System.currentTimeMillis()));
		transactionData.setReconCyclenNo(new BigDecimal(item.getReconCycleNo().intValue()+1));
		return transactionData;
	}
	
	private RawData formRawData(ReconBOU item) {
	...
	private void process(ReconBOU item) throws Exception {
		ReconDetails reconDetails = new ReconDetails();
		/*
		...
		if (item.getRawRefId() == null) {
			...
			if(item.getReconCycleNo()!=null && ((item.getReconCycleNo().doubleValue()-reconMaxTry)>=0)) {
				...
				BillerOUtransactionData transactionData = formTransactionData(item);
	...
	private RawDataView formRDCU(ReconBOU item) {
		if (item == null) {
			return null;
		}
	...
	private RawDataView formRDOU(ReconBOU item) {
			...
			rdv.setReversal(item.getCuStatus().equals(RequestStatus.RESPONSE_REVERSE.name()));
			rdv.setDecline(item.getCuStatus().equals(RequestStatus.RESPONSE_DECLINE.name()));

				if (item.getCuRequestJson() != null) {
					BillPaymentRequest billPaymentRequest = null;
					try {
						billPaymentRequest = objectMapper.readValue(item.getCuRequestJson(), BillPaymentRequest.class);
				...
				if (item.getCuResponseJson() != null) {
					BillPaymentResponse billPaymentResponse = null;
					try {
						billPaymentResponse = objectMapper .readValue(item.getCuResponseJson(), BillPaymentResponse.class);
					} catch (IOException e) {
						e.printStackTrace();
					}
	...
	@Override
	public void write(List<? extends ReconBOU> items) throws Exception {
		if(items!=null){
				for(ReconBOU data: items)
				{
					process(data);
